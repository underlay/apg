...
export function fold(m1, type, target) {
    if (type.type === "reference") {
        ...
        for (const { key, value } of type.options) {
            options.push(Object.freeze({ type: "option", key, value: fold(m1, value, target) }));
        }
...
export function delta(mapping, source, target, instance) {
    const [m1, m2] = mapping;
    const result = m1.map(() => []);
...
instance, indices) {
    ...
    if (sourceType.type === "reference") {
        ...
        if (index === undefined) {
            ...
            instance[sourceType.value][index] = pullback(m1, 
            // t2,
        ...
        if (image.termType !== "Record") {
            ...
            return new APG.Record(image.node, image.componentKeys, image.map((field, i) => {
        ...
        if (image.termType !== "Variant") {
            ...
            return new APG.Variant(image.node, image.optionKeys, image.index, pullback(m1, value, source, target, image.value, instance, indices));
        }
    }
    else {
        signalInvalidType(sourceType);
