...
(function (APG) {
    ...
    APG.ProductValue = ProductValue;
    class CoproductValue {
        constructor(node, option, value) {
            this.node = node;
            this.option = option;
            this.value = value;
        }
    ...
    APG.CoproductValue = CoproductValue;
    APG.toId = (id) => `_:${id}`;
    APG.toValue = (id, schema) => {
        if (typeof id === "string") {
            const { type } = schema.types.get(id);
            return { [type]: APG.toId(id) };
        }
        else {
            return { reference: { type: "reference", value: APG.toId(id.value) } };
        }
    };
    function toJSON(schema) {
        ...
        for (const [id, label] of schema.labels) {
            graph.push({
                ...
                type: "label",
                key: label.key,
                value: APG.toValue(label.value, schema),
            });
        }
        for (const [id, type] of schema.types) {
            if (type.type === "unit") {
                ...
                graph.push({ id: APG.toId(id), type: "literal", datatype: type.datatype });
            }
                ...
                for (const [id, { key, value }] of type.components) {
                    components.push({
                        ...
                        type: "component",
                        key,
                        value: APG.toValue(value, schema),
                    });
                }
