...
const isOptionalProperty = type => isProperty(type) || type.type === "coproduct" && _index.ns.none in type.options && type.options[_index.ns.none].type === "unit" && _index.ns.some in type.options && isProperty(type.options[_index.ns.some]);

function isRelationalSchema(input) {
  for (const [key, type] of (0, _utils.getEntries)(input)) {
    if (type.type === "unit") {
      continue;
    } else if (type.type === "product") {
      for (const [_, value] of (0, _utils.getEntries)(type.components)) {
