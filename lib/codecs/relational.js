...
const optionalProperty = t.union([property, t.type({
  ...
  options: t.type({
    [_index.ns.none]: t.type({
      type: t.literal("product"),
      components: t.type({})
    }),
    [_index.ns.some]: property
  })
})]);
const type = t.type({
  type: t.literal("product"),
...
const isOptionalProperty = type => isProperty(type) || type.type === "coproduct" && (0, _index.getKeys)(type).length === 2 && _index.ns.none in type.options && (0, _unit.isUnit)(type.options[_index.ns.none]) && _index.ns.some in type.options && isProperty(type.options[_index.ns.some]);

function isRelationalSchema(input) {
  for (const [{}, type] of (0, _index.forEntries)(input)) {
    if (type.type === "product") {
      for (const [_, value] of (0, _index.forEntries)(type.components)) {
