...
function map(morphism, value, instance) {
  if (morphism.type === "identity") {
    ...
    if (value.termType === "Record" && value.componentKeys[morphism.index] === morphism.componentKeys[morphism.index]) {
    ...
    if (value.termType === "Variant" && value.optionKeys[value.index] === morphism.optionKeys[value.index]) {
    ...
    return new _apg.default.Record(new N3.BlankNode((0, _utils.getId)()), morphism.componentKeys, morphism.morphisms.map(morphism => map(morphism, value, instance)));
  } else if (morphism.type === "injection") {
    return new _apg.default.Variant(new N3.BlankNode((0, _utils.getId)()), morphism.optionKeys, morphism.index, value);
  } else {
    (0, _utils.signalInvalidType)(morphism);
  }
}

function delta(mapping, source, target, instance) {
  const [m1, m2] = mapping;
  const result = m1.map(() => []);
  ...
  for (const [{
    value: type
  }, path, morphism, i] of (0, _ziterable.default)(source, m1, m2)) {
    for (const value of (0, _path.getValues)(instance, path)) {
      if (indices[i].has(value)) {
        ...
        result[i][index] = pullback(m1, type, source, target, image, result, indices);
      }
    }
  }
...
function pullback(m1, // sourceType: APG.Type,
sourceType, // imageType
source, target, image, // neither
instance, indices) {
  console.log("pulling it back", sourceType, image); // if (sourceType.type === "reference") {
  ...
  if (sourceType.type === "reference") {
    ...
    const t2 = (0, _path.getType)(target, m1[sourceType.value]);
    ...
    if (index === undefined) {
      ...
      instance[sourceType.value][index] = pullback(m1, // t2,
      source[sourceType.value].value, source, target, image, instance, indices);
      return new _apg.default.Pointer(index, sourceType.value);
    } else {
      return new _apg.default.Pointer(index, sourceType.value);
    }
  } else if (sourceType.type === "unit") {
    ...
    if (image.termType !== "Record") {
      ...
      return new _apg.default.Record(image.node, image.componentKeys, image.map((field, i) => {
        const {
          value
        } = sourceType.components[i];
    ...
    if (image.termType !== "Variant") {
      ...
      const {
        value
      } = sourceType.options[image.index];
      return new _apg.default.Variant(image.node, image.optionKeys, image.index, pullback(m1, value, source, target, image.value, instance, indices));
    }
  } else {
    (0, _utils.signalInvalidType)(sourceType);
  }
}