...
function validateMapping(M, S, T) {
  for (const key of (0, _utils.getKeys)(S)) {
    ...
    const source = (0, _path.getType)(T, M[key].source, M[key].target);
    const target = fold(M, S, T, value);

    if ((0, _morphism.validateExpressions)(S, M[key].value, source, target)) {
      continue;
    } else {
      return false;
    }
...
function fold(M, S, T, type) {
  if (type.type === "reference") {
    ...
    return Object.freeze({
      type: "product",
      components: (0, _utils.mapKeys)(type.components, value => fold(M, S, T, value))
    });
  } else if (type.type === "coproduct") {
    return Object.freeze({
      type: "coproduct",
      options: (0, _utils.mapKeys)(type.options, value => fold(M, S, T, value))
    });
  } else {
...
function map(expression, value, instance, schema) {
  if (expression.type === "identity") {
    ...
    return new _apg.default.Variant(expression.key, mapExpressions(expression.value, value, instance, schema));
  } else {
...
function delta(M, S, T, TI) {
  ...
  for (const [key, type] of (0, _utils.getEntries)(S)) {
    if (!(key in M) || !(key in indices)) {
      throw new Error("Invalid mapping");
    }

    for (const value of (0, _path.getValues)(T, TI, M[key].source, M[key].target)) {
...
value // of image
  ...
  if (type.type === "reference") {
    ...
    if (value.termType !== "Variant") {
      ...
      return new _apg.default.Variant(value.option, pullback(state, type.options[value.option], value.value));
    } else {
