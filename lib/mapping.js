...
value // of image
  ...
  if (type.type === "reference") {
    ...
    if (value.termType !== "Record") {
      ...
      return new _apg.default.Record(value.node, value.componentKeys, pullbackComponents(M, S, T, SI, TI, indices, type, value));
    }
  } else if (type.type === "coproduct") {
    if (value.termType !== "Variant") {
      ...
      return new _apg.default.Variant(value.node, value.optionKeys, value.index, pullback(M, S, T, SI, TI, indices, t, value.value));
    }
  } else {
    (0, _utils.signalInvalidType)(type);
  }
