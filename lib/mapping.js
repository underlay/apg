...
function map(expression, value, instance, schema) {
  if (expression.type === "identity") {
    ...
    return new _apg.default.Variant(Object.freeze([expression.key]), 0, mapExpressions(expression.value, value, instance, schema));
  } else {
...
value // of image
  ...
  if (type.type === "reference") {
    ...
    if (value.termType !== "Variant") {
      ...
      return new _apg.default.Variant(value.options, value.index, pullback(state, type.options[value.option], value.value));
    } else {
