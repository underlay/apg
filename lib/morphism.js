...
function apply(S, expression, source) {
  if (expression.type === "identity") {
    ...
    return _apg.default.literal(expression.datatype);
    ...
    return _apg.default.coproduct({
      [key]: value.reduce((type, expression) => apply(S, expression, type), source)
    });
  } else if (expression.type === "tuple") {
    return _apg.default.product((0, _utils.mapKeys)(expression.slots, value => applyExpressions(S, value, source)));
