...
function getType(schema, [label, nil, ...path]) {
  ...
  return path.reduce((type, index) => {
    if (type.type === "product" && index >= 0 && index < type.components.length) {
      return type.components[index].value;
    } else if (type.type === "coproduct" && index < 0 && index >= -type.options.length) {
...
function* getValues(instance, [label, nil, ...path]) {
  ...
  for (const element of instance[label]) {
    const value = path.reduce((value, index) => {
      if (value === null) {
        return null;
      } else if (value.termType === "Record" && index >= 0 && index < value.length) {
        return value[index];
      } else if (value.termType === "Variant" && index < 0 && index >= -value.optionKeys.length) {
