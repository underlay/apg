...
function getType(schema, [label, nil, ...path]) {
...
function* getValues(schema, instance, [label, nil, ...path]) {
  ...
  for (const element of instance[label]) {
    const token = path.reduce((token, index) => {
      if (token === null) {
        return null;
      }
      ...
      if (type.type === "product" && index in type.components && value.termType === "Record" && index in value) {
        return [type.components[index].value, value[index]];
      } else if (type.type === "coproduct" && -1 - index in type.options && value.termType === "Variant") {
        if (value.key === type.options[-1 - index].key) {
          return [type.options[-1 - index].value, value.value];
        } else {
          return null;
        }
      } else if (type.type === "reference" && type.value in schema && type.value in instance && value.termType === "Pointer" && value.index in instance[type.value] && isNaN(index)) {
