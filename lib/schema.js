...
var _schema = _interopRequireDefault(require("./schemas/schema.js"));
...
function toSchema(instance) {
  if (instance.length !== _schema.default.length) {
    throw new Error("Invalid schema schema instance");
  }

  const database = new Map(_schema.default.map((label, index) => [label.key, instance[index]]));
  ...
  const typeCache = new Map(_schema.default.map(({
    key
  }) => [key, new Map()]));
...
const schemaIndices = new Map(_schema.default.map(({
  key
}, i) => [key, i]));
...
function fromSchema(schema) {
  let id = 0;
  const database = new Map(_schema.default.map(({
    key
  }) => [key, []]));
  const cache = new Map();
  const labels = database.get(ns.label);
  ...
  const instance = _schema.default.map(({
    key
  }) => {
