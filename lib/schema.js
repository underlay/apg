...
export function parseSchema(store, schemaSchema) {
    ...
    for (const { value: labelValue } of components) {
        const { node: { id: componentId }, components: [sourceValue, keyValue, valueValue], } = labelValue;
        const { value: sourceLabelValue } = sourceValue;
        const { node: { id: valueId }, } = sourceLabelValue;
        const { node: { value: key }, } = keyValue;
        const { id: value } = parseReference(valueValue, dictionary);
        const product = schema.get(valueId);
    ...
    for (const { value: labelValue } of options) {
        const { node: { id: optionId }, components: [sourceValue, valueValue], } = labelValue;
        const { value: sourceLabelValue } = sourceValue;
        const { node: { id: valueId }, } = sourceLabelValue;
        const { id: value } = parseReference(valueValue, dictionary);
        const coproduct = schema.get(valueId);
