...
function toType(value, instance, typeCache, sources, permutation) {
  if (value.is(ns.reference)) {
    ...
    return _apg.default.reference(permutation.get(index));
  } else if (value.is(ns.uri)) {
    return _apg.default.uri();
  } else if (value.is(ns.literal)) {
    const {
      value: datatype
    } = value.value;
    return _apg.default.literal(datatype);
...
function toProduct(index, instance, typeCache, sources, permutation) {
  ...
  const product = _apg.default.product(components === undefined ? {} : Object.fromEntries(components.map(component => {
    const {
      value: key
...
function toCoproduct(index, instance, typeCache, sources, permutation) {
  ...
  const coproduct = _apg.default.coproduct(options);
...
function fromSchema(schema) {
  ...
  for (const key of (0, _utils.getKeys)(schema)) {
    ...
    const variant = new _apg.default.Variant(valueKeys, ns[type.type], fromType(schema, instance, cache, type));
    instance[ns.label].push(new _apg.default.Record(labelKeys, [new N3.NamedNode(key), variant]));
  }
...
function fromType(schema, instance, cache, type) {
  if (type.type === "reference") {
    return new _apg.default.Pointer((0, _utils.getKeyIndex)(schema, type.value));
  } else if (type.type === "uri") {
    return _apg.default.unit();
    ...
    const index = instance[ns.product].push(_apg.default.unit()) - 1;
    cache.set(type, index);

    for (const [key, value] of (0, _utils.forEntries)(type.components)) {
      instance[ns.component].push(new _apg.default.Record(componentKeys, [new N3.NamedNode(key), new _apg.default.Pointer(index), new _apg.default.Variant(valueKeys, ns[value.type], fromType(schema, instance, cache, value))]));
    }
    ...
    const index = instance[ns.coproduct].push(_apg.default.unit()) - 1;
    cache.set(type, index);

    for (const [key, value] of (0, _utils.forEntries)(type.options)) {
      instance[ns.option].push(new _apg.default.Record(optionKeys, [new N3.NamedNode(key), new _apg.default.Pointer(index), new _apg.default.Variant(valueKeys, ns[value.type], fromType(schema, instance, cache, value))]));
    }
