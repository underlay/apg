...
function fromSchema(schema) {
  ...
  for (const key of (0, _utils.getKeys)(schema)) {
    ...
    const variant = new _apg.default.Variant(valueKeys, getValueIndex(type.type), fromType(schema, instance, cache, id, type));
...
function fromType(schema, instance, cache, id, type) {
  if (type.type === "reference") {
    ...
    for (const [key, value] of (0, _utils.forEntries)(type.components)) {
      instance[ns.component].push(new _apg.default.Record(componentKeys, [new N3.NamedNode(key), new _apg.default.Pointer(index), new _apg.default.Variant(valueKeys, getValueIndex(value.type), fromType(schema, instance, cache, id, value))]));
    }
    ...
    for (const [key, value] of (0, _utils.forEntries)(type.options)) {
      instance[ns.option].push(new _apg.default.Record(optionKeys, [new N3.NamedNode(key), new _apg.default.Pointer(index), new _apg.default.Variant(valueKeys, getValueIndex(value.type), fromType(schema, instance, cache, id, value))]));
    }
