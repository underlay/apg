...
export function makeShExSchema(labels) {
...
export function parseSchema(store, schemaSchema) {
    ...
    for (const optionType of map.get(ns.option).values()) {
        if (optionType.termType === "Tree") {
            ...
            const option = { type: "option", value: { id: optionType.value } };
...
function parseResult(node, type, result, shapeExpr, state) {
    if (isFailure(result)) {
        ...
        console.log("result!", JSON.stringify(result, null, "  "), isNilShapeResult(result));
