...
export function parse(store, schema) {
    ...
    const state = Object.freeze({ schema, stack: [], cache: new Map() });
    for (const [id, label] of schema.labels) {
        ...
        for (const subject of subjects) {
            if (subject instanceof BlankNode) {
                ...
                const match = parseResult(subject, { type: "reference", value: id }, result, state);
                if (match._tag === "None") {
                    const errors = [
...
function parseResult(node, value, result, state) {
...
function parseReferenceResult(node, id, label, result, state) {
...
function replaceToken(node, value, uri) {
    if (value.termType === "Product") {
        ...
        const coproduct = new APG.CoproductValue(value.node, value.option, node);
...
function replaceTokenValue(root, value, uri) {
    if (value.termType === "Product") {
        const product = new APG.ProductValue(value.node);
        ...
        const coproduct = new APG.CoproductValue(value.node, value.option, root);
...
function* replaceLeaves(root, product, uri) {
    for (const [id, leaf] of product) {
        yield [id, replaceTokenValue(root, leaf, uri)];
    }
}
function parseTypeResult(node, id, type, result, state) {
    if (type.type === "unit") {
