...
type Entry = [string, Instance.Value[]]
const mapValues = (value: Instance.Value) => value.toJSON()
const mapEntry = ([key, values]: Entry) => [key, values.map(mapValues)]

yargs(hideBin(process.argv))
	.usage("Usage: $0 <command> [options]")
	.command(
		"cat",
		"Print a JSON representation of a schema or instance",
		(yargs) =>
			yargs
				.option("schema", {
					alias: "s",
