...
import { zip, signalInvalidType, getKeys, forEntries, mapKeys, } from "../utils.js";
...
export function map(expression, value, instance, schema) {
    if (expression.kind === "identifier") {
        ...
        return Instance.product(keys, keys.map((key) => mapExpressions(expression.slots[key], value, instance, schema)));
    }
    else if (expression.kind === "injection") {
        return Instance.coproduct(Object.freeze([expression.key]), expression.key, value);
    }
    else {
...
value // of image
    ...
    if (Schema.isReference(type)) {
        ...
        if (value.kind !== "product") {
            ...
            return Instance.product(value.components, pullbackComponents(state, type, value));
        }
    }
        ...
        if (value.kind !== "coproduct") {
            ...
            return Instance.coproduct(value.options, value.key, pullback(state, type.options[value.key], value.value));
        }
        else {
